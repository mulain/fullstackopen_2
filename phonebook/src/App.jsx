...
const App = () => {
    ...
    const handleGetContacts = () => {
        contactService
            ...
            // })
            .then(setContacts) // works, because setPersons takes one argument
            .catch((error) => {
    ...
    const handleUpdateContact = () => {
        if (
            ...
            contactService
                ...
                .catch((error) => {
                    handleNotify(
                        `Error updating contact "${contactToUpdate.name}"`,
                        "error"
                    )
                    handleGetContacts()
                })
    ...
    const handleAddContact = () => {
        contactService
            ...
            .catch((error) => {
                handleNotify(`Error adding contact "${newName}"`, "error")
                handleGetContacts()
            })
    }
    ...
    const handleDelete = (contact) => {
        if (window.confirm(`Delete contact ${contact.name} with id ${contact.id}?`)) {
            contactService
                ...
                .catch((error) => {
                    handleNotify(`Error deleting contact "${contact.name}"`, "error")
                    handleGetContacts()
                })
        }
    }
